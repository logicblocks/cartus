...
(deftest clear-events!-test
  ...
  (testing "Can clear events for transformer logger"
    (let [tx-logger (cartus-core/with-transformation
      ...
      (is (= 0
            (-> tx-logger cartus-test/events count)))))
  (testing "Can fetch and clear events for composed logger"
    (let [non-test-logger (reify Object
                            cartus-core/Logger
                            (log [_ _ _ _ _]
                              "Do nothing"))
          logger-a (cartus-test/logger)
          logger-b (cartus-test/logger)
          logger (cartus-core/compose-loggers
                   logger-a
                   logger-b
                   non-test-logger)]
      (cartus-core/info logger ::some-event {})
      (cartus-core/debug logger ::some-other-event {})
      (testing "accumulates logs from test loggers"
        (is (= 4
              (-> logger cartus-test/events count))))
      (testing "each test logger is logged to"
        (is (= 2
              (-> logger-a cartus-test/events count)))
        (is (= 2
              (-> logger-b cartus-test/events count))))
      (cartus-test/clear-events! logger)
      (testing "no logs are returned"
        (is (= 0
              (-> logger cartus-test/events count))))
      (testing "logs have been cleared from each sub-logger"
        (is (= 0
              (-> logger-a cartus-test/events count)))
        (is (= 0
              (-> logger-b cartus-test/events count)))))))
